...
class StockPortfolioEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.day += 1
            self.data = self.df.loc[self.day,:]
            self.data = self.data.fillna(method='ffill')  # You can also use 'mean' for average imputation
            state_elements = [np.array(self.covs).reshape(1, -1)]  # Reshape covs to 1D
            for tech in self.tech_indicator_list:
                state_elements.append(self.data[tech].values.reshape(1, -1))  # Reshape each tech data to 1D
            self.state = np.column_stack(state_elements)
